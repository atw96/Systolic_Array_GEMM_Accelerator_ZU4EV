...
│                           #   (base addr 0x8005_0000, ARRAY_SIZE default 16)
├── tb/
...
│   ├── run_xsim_full.sh    # Full systolic array simulation (N=16, fixed AXI tasks)
...
### 1. Array scale-up: 4×4 → 8×8 → 16×16 (parameterized)
...
| **16 (this variant)** | **256** | **35.2%** | **~19,000 (est.)** | 290 | Feasible; comfortably under the 60% LUT budget noted in the hardware config doc |
| 24 | 576 | 79.1% | ~43,000 (est.) | 626 | Tight; needs careful floorplanning and timing closure |
...
// rtl/axi_ctrl_top.v
module axi_ctrl_top #(
    parameter integer ARRAY_SIZE = 16,   // change this (this variant ships with 16)
    ...
```
...
For this variant's N=16: `RESULT_0..15 @ 0x08-0x44`, `DATA_IN_0..15 @ 0x48-0x84`,
`WEIGHT_00..1515 @ 0x88-0x484` (290 registers, 1160 bytes total — well inside the
...
python3 python/host_eth_receiver.py --port 9000 --array_size 16
...
# 3. Run full systolic array simulation (default N=16, fixed AXI task timing)
...
#    ARRAY_SIZE parameter — both default to 16)
cd ..
python3 python/golden_model.py --array_size 16 --test_id 0 --mode sim
# Expected: ✓ [PASS]
```
...
| Resource | Used (N=16, this variant) | Available | Utilization |
|----------|------|-----------|-------------|
| DSP48E2 | 256 | 728 | **35.2%** |
| LUT | ~19,000 (est.*) | 88,320 | ~21.5% (est.) |
| FF | ~24,000 (est.*) | 176,640 | ~13.6% (est.) |
| BRAM | 0 | 360 | 0% |
...
```
Expected output from test_app (N=16, running all 5 built-in test vectors):
======================================================
  INT8 脉动阵列 GEMM (16x16) - 板卡验证程序（ACU4EV 优化版）
...
31 for this variant's 16×16. The skewing chains are `generate`-loop based so
...
transfers well under 1 KB per inference at N=16 (290 registers × 4 bytes = 1160 bytes) — AXI4-Lite
overhead is negligible and the implementation is far simpler to verify.

**Q4: Why 16×16 for this variant instead of 8×8 or 24×24?**

`ARRAY_SIZE` is a single parameter, so three sibling packages were produced side by side
(8×8, 16×16, 24×24) rather than picking one "winner" — the right size depends on what
you're optimizing for. 16×16 sits at a specific point on that curve: it uses 35.2% of the
board's 728 DSP48E2 slices (256 of them), which is a 4× throughput increase over 8×8 while
still leaving well over half the DSP fabric free for other logic (e.g. a second accelerator,
or headroom for a future streaming datapath). The AXI register map grows to 290 registers
(1,160 bytes) — still small enough that a full weight+data load is a modest, easy-to-reason-about
sequence of AXI transactions, unlike 24×24's 626 registers. Estimated LUT usage (~19,000,
~21.5%) is comfortably under the 60% budget noted in the hardware config doc, so timing
closure risk should be low relative to 24×24. In short: 16×16 is the "confident middle"
size — a large step up from 8×8 in compute density, without yet entering the tighter
floorplanning/timing-closure territory that 24×24 starts to require.
